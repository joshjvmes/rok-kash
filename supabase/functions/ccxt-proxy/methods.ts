...
export async function executeExchangeMethod(
  ...
  try {
    ...
    switch (method) {
      ...
      case 'fetchBalance':
        console.log(`Fetching balance for ${exchange.id}...`);
        try {
          const balance = await exchange.fetchBalance();
          console.log(`Successfully fetched balance for ${exchange.id}`);
          ...
          console.error(`Error fetching ${exchange.id} balance:`, error);
          throw error;
        }
