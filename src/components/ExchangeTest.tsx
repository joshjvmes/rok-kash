...
interface ExchangeTestProps {
  exchange: string;
}

export function ExchangeTest({ exchange }: ExchangeTestProps) {
  ...
  useEffect(() => {
    async function fetchInitialPairs() {
      try {
        ...
        console.log(`First 10 available pairs for ${exchange}:`, spotPairs);
        setPairs(spotPairs);
        setIsLoading(false);
      } catch (error) {
        ...
        toast({
          ...
          description: `Could not fetch trading pairs from ${exchange}`
        });
        setIsLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const updatePrice = async () => {
      try {
        ...
        const price = await fetchCCXTPrice(exchange.toLowerCase(), currentPair.symbol);
  ...
  }, [currentPairIndex, pairs.length, isLoading, exchange]);

  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">{exchange} Trading Pairs</h1>
      <Card className="p-4">
