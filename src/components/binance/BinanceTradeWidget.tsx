...
export function BinanceTradeWidget() {
  ...
  const { data: balanceData, isLoading: isLoadingBalance } = useQuery({
    ...
    queryFn: () => fetchBalance('binance') as Promise<Balance>,
    ...
    const fetchPrice = async () => {
      if (selectedPair) {
        try {
          ...
          const price = await fetchCCXTPrice('binance', selectedPair);
          if (isMounted) {
  ...
  const handleCancelOrder = async () => {
    ...
    try {
      ...
      await cancelOrder('binance', activeOrder.id, activeOrder.symbol);
  ...
  const handleTrade = async (side: 'buy' | 'sell') => {
    ...
    try {
      const order = await createOrder('binance', selectedPair, 'market', side, numericAmount) as OrderResponse;
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <Card className="p-6">
        <h2 className="text-lg font-semibold mb-4">Binance Trade</h2>
