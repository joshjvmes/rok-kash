...
export function KrakenAccountInfo() {
  ...
  const { data: balance, isLoading } = useQuery<BalanceData>({
    queryKey: ['balance', 'kraken'],
    queryFn: () => fetchBalance('kraken'),
        ...
        if (amount > 0) {
          try {
            ...
            if (!['USDT', 'USDC', 'DAI'].includes(coin)) {
              const price = await fetchCCXTPrice('kraken', `${coin}/USDT`);
