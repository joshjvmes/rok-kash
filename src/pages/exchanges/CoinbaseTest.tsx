...
export default function CoinbaseTest() {
  ...
  useEffect(() => {
    async function fetchInitialPairs() {
      try {
        ...
        console.error('Error fetching Coinbase pairs:', error);
        toast({
          variant: "destructive",
          title: "Error fetching pairs",
          description: "Could not fetch trading pairs from Coinbase"
        });
        setIsLoading(false);
      }
    }
  ...
  useEffect(() => {
    ...
    const updatePrice = async () => {
      try {
        ...
        const price = await fetchCCXTPrice('coinbase', currentPair.symbol);
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Coinbase Trading Pairs</h1>
      <Card className="p-4">
